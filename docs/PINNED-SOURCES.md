...
| **Codrops wave, ranks 21-36 (fx-cd-d)** — resolved 2026-09-06 from `CANDIDATES.md`, re-fetched through `GET repos/{repo}/contents/{path}?ref={sha}` before porting, 30/30 paths returned bytes | | | | | |
| text / text-repetition | github.com/codrops/TextRepetitionEffect | `fabaafe0124e7bdf66906c6214f00800f5575c4c` | `src/js/demo1/repeatTextScrollFx.js` **+ `src/js/utils.js`** (`getHeight`) **+ `src/css/base.css`** | MIT | LICENSE present, 1103 B, full MIT text; README carries no restrictive clause |
| text / sliced-text | github.com/codrops/SlicedTextEffect | `45bbcfd96b0c87a5a5299cb4bacc583dd3b71bf8` | `js/item.js` **+ `js/index.js`** (the `fx1Timeline`; `item.js` is layout only) **+ `css/base.css`** | MIT | same |
| text / circular-text | github.com/codrops/CircularTextEffect | `fcd9aaa5d55d0d6ce5f9d1b6795e4c3d45dd93b2` | `src/js/demo1/intro.js` **+ `src/index.html`** (the four circle paths and their `textLength`) **+ `src/css/base.css`** (`transform-origin: 700px 700px`) | MIT | same |
| text / terminal-hover | github.com/codrops/LineTextHoverAnimations | `00fdd50a5daa48a9fcb400d19d17b1dac5935e68` | `js/effect-1/text-animator.js` **+ `js/effect-1/index.js`** (the row-wide trigger) **+ `js/textSplitter.js` + `css/base.css`** (the `1ch` block cursor) | MIT | same |
| cursor / image-trail | github.com/codrops/codrops-sketches | `bbf47ca34766fd2ca5f97d8b376d2eca148bf48f` | `005-image-motion-trail-opaque/js/index.js` **+ `.../css/base.css`** | MIT | LICENSE present, 1064 B, MIT © 2022 Codrops. **No GSAP at this path** — the sketch is a lerp plus `requestAnimationFrame`, so the port needs no vendor key at all |
| cursor / pixel-tooltip | github.com/codrops/PixelGooeyTooltip | `60ade47c343adbe40bdfa1bb0210a898e03ca536` | `js/tooltip.js` **+ `js/index.js`** (the 40ms debounce) **+ `css/tooltip.css` + `index.html`** | MIT | LICENSE present, 1103 B. **The `#gooey` filter in `index.html` is never referenced** by either stylesheet at this commit, so the name overstates what the demo shows |
| gallery / clip-hover | github.com/codrops/ClipHoverEffect | `57b5cc1b6cc375e69dd915bdd307f2645e1dabb7` | `js/card.js` **+ `js/utils.js`** (`lettersAndSymbols`) **+ `css/base.css`** (`.js .card__img { opacity: 0 }`) | MIT | same |
| gallery / grid-3d-stagger | github.com/codrops/Staggered3DGridAnimations | `7c2703d41f55005c2758e1031b593d2493d392f8` | `js/index.js` (`animateScrollGrid` only) **+ `index.html` + `css/base.css`** (per-item `perspective: 800px`) | MIT | same. The other three functions in that file need `SplitText`, a Club GreenSock plugin, and are not ported |
| menu / expanding-menu | github.com/codrops/ExpandingRoundedMenu | `9f8174aaca176a0e4b31e1330f9049b45fd02758` | `src/js/index.js` **+ `src/index.html` + `src/css/base.css`** (the two-layer cover: `.cover` at -100% with `.cover__inner` at +100% inside it) | MIT | same |
| scroll / card-stack-scroll | github.com/codrops/3DStackMotion | `75cbda91ed26d31aeeb5a5b9887848e5a7005c7f` | `js/effect-1/stackMotionEffect.js` **+ `js/utils.js`** (`throttle`) **+ `index.html` + `css/base.css`** | MIT | same. **Carries a bug at this commit:** `this.imageElements = [nodeList]`, so `imagesTotal` is 1 whatever the deck holds — declared as `kind: "upstream-bug"` |
