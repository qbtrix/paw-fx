...
`bun run gallery` builds the registry and then writes `dist/registry/gallery/`: a documentation site carrying every effect, its preview, what it needs, which engines take it, the licence and the upstream file at the pinned commit, the deviations it declares, its options, and the `get_effect` call that fetches it. The audience is someone picking an effect for a client site.

The shape is a slim top bar (name, count, a search field with a `/` shortcut), a persistent left sidebar, and a dense card grid as the main column: four across on a laptop, two on a tablet, one on a phone. The sidebar is nine collapsible groups, one per category, each with its count and the effects inside it, plus a group of the filters that cut across all of them. Under 900px it becomes a drawer over the content, with the page behind it `inert` and a focus trap inside it, rather than disappearing. Groups, counts and per-effect links are all derived from the items, so a tenth category appears on its own.
...
Previews ride inside `index.html` as `data:` URIs. An html Paw Site is created from a `{path: contents}` map whose values must be strings, so binary has no way in, and the page has to be one publishable map. `sips` re-encodes each 640x360 PNG to JPEG first: the PNGs total about 9 MB, which is well past the 4 MB deploy-input cap a publish captures. The quality is the knob that keeps the page publishable and it moves down as the library grows -- base64 adds a third on top of the JPEGs, so each effect costs about 45 KB at quality 75. It sat at 75 through 73 effects and crossed the cap at 77, then at 65 through 84 and crossed again at 85; `JPEG_QUALITY` in `scripts/build-gallery.mjs` is 56 now, which is 3.58 MB. A dense particle field costs about three times what a flat gradient does at the same quality, so the number moves faster as the particles shelf fills. `tests/gallery.test.js` asserts the cap, so this fails the build rather than failing a publish. Without `sips` (a Linux CI, say) the PNG ships as-is: the same page, five times the bytes, and too big to publish.
Previews ride inside `index.html` as `data:` URIs. An html Paw Site is created from a `{path: contents}` map whose values must be strings, so binary has no way in, and the page has to be one publishable map. `sips` re-encodes each 640x360 PNG to JPEG first: the PNGs total about 9 MB, which is well past the 4 MB deploy-input cap a publish captures. The quality is the knob that keeps the page publishable and it moves down as the library grows -- base64 adds a third on top of the JPEGs, so each effect costs about 45 KB at quality 75. It sat at 75 through 73 effects and crossed the cap at 77; `JPEG_QUALITY` in `scripts/build-gallery.mjs` is 65 now. Measured at 65 on these 77: each preview is 37 KB of the page, the whole shell of markup around them is 718 KB, and the doc-site chrome added 27 KB of that when it replaced the one-page layout. `index.html` is 3.66 MB, so the headroom is about 340 KB, or nine more effects. `tests/gallery.test.js` asserts the cap, so this fails the build rather than failing a publish. When that headroom runs out, moving the previews to sibling files is the better fix than dropping the quality again -- there is already a `demo/` tree beside `index.html` -- but it trades away the single-string-map publish this section describes, so check that path before taking it. Without `sips` (a Linux CI, say) the PNG ships as-is: the same page, five times the bytes, and too big to publish.

Cards, sidebar and panels are rendered into the HTML rather than by the browser, so the page reads with scripting off and every effect is there for a test to count. `gallery.js` only filters, counts, ranks (the same order `search_effects` returns), opens and copies.

The fragment carries the view, under one grammar that keeps filters and panels from colliding: `#<name>` opens that effect's panel, which is the `preview_url` contract the MCP server hands agents; `#cat=<category>` and `#free` are filters, and `#cat=text&free` stacks them. A bare token is an effect id and a `k=v` token is a filter, so the only name that could shadow a filter is an effect literally called `free`, and the generator throws on one rather than leaving it to be found in the wild. Every category and filter in the sidebar is a plain anchor, so a view is linkable and the back button works; closing a panel puts the filter hash back rather than the bare path. The sidebar counts are facets of the SEARCH only, never of the active category, so a search tells you how many hits sit down each road instead of zeroing the eight roads not taken.

No live previews **on the grid page**. A document holds roughly 8 to 16 WebGL contexts before it starts evicting the oldest, and 29 of them would leave a grid of dead canvases, which is worse than a grid of images. The banner is the one live shader there. Every card and every panel instead links to that effect's own demo page, which is where the live version lives.
